...
$page_title = "รายงานเงินนอกงบประมาณ - AMSS++";
$current_page = basename($_SERVER['PHP_SELF']); 
// ชื่อหน้าบนแถบสถานะ
$page_header = 'รายงานเงินนอกงบประมาณ';

// --- ดึงข้อมูล ---
$sql_data = "SELECT * FROM off_budget_report_data ORDER BY id ASC";
...
$sql_sum = "SELECT 
    ...
    (SELECT balance_cash FROM off_budget_report_data ORDER BY id DESC LIMIT 1) as last_cash,
    (SELECT balance_bank FROM off_budget_report_data ORDER BY id DESC LIMIT 1) as last_bank,
    (SELECT balance_gov FROM off_budget_report_data ORDER BY id DESC LIMIT 1) as last_gov
    FROM off_budget_report_data";
...
<style>
    ...
    .dropdown-item[href*="Report money outside the budget.php"] {
...
<div class="container-fluid pb-5 px-3">
    <div class="content-card">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            <h2 class="page-title-custom">รายงานเงินนอกงบประมาณ</h2>
            ...
            <div class="input-group shadow-sm" style="width: auto; border-radius: 6px; overflow: hidden;">
                ...
                <select class="form-select form-select-sm border-start-0" style="width: 150px; cursor: pointer; font-size: 0.9rem;">
            ...
            <table class="table table-custom">
                <thead>
                    <tr>
                        ...
                        <th rowspan="2" style="width: 10%; border-bottom: 1px solid #e2e8f0;">ที่เอกสาร</th>
                        <th rowspan="2" style="width: 25%; border-bottom: 1px solid #e2e8f0;">รายการ</th>
                        <th rowspan="2" style="width: 12%; border-bottom: 1px solid #e2e8f0;">ลักษณะรายการ</th>
                ...
                <tbody>
                    ...
                    if ($result_data && $result_data->num_rows > 0) {
                        ...
                        while($row = $result_data->fetch_assoc()) {
                            ...
                            echo "<td class='td-right'>" . ($row['transfer_amount'] > 0 ? number_format($row['transfer_amount'], 2) : '') . "</td>";
                            echo "<td class='td-right fw-bold text-success'>" . ($row['receive_amount'] > 0 ? number_format($row['receive_amount'], 2) : '') . "</td>";
                            echo "<td class='td-right fw-bold text-danger'>" . ($row['pay_amount'] > 0 ? number_format($row['pay_amount'], 2) : '') . "</td>";
                            
                            // คงเหลือ
                            echo "<td class='td-right' style='color: #0f172a;'>" . number_format($row['balance_cash'], 2) . "</td>";
                            echo "<td class='td-right' style='color: #0f172a;'>" . number_format($row['balance_bank'], 2) . "</td>";
                            echo "<td class='td-right' style='color: #0f172a;'>" . number_format($row['balance_gov'], 2) . "</td>";
                        ...
                        echo "<tr><td colspan='12' class='text-center py-5 text-muted'>ไม่พบข้อมูลเงินนอกงบประมาณ</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
