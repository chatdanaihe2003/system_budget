...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'add') {
        $stmt = $conn->prepare("INSERT INTO receive_national (receive_order, doc_date, doc_no, description, transaction_type, amount, file_name) VALUES (?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($file_name) {
            ...
            $stmt = $conn->prepare("UPDATE receive_national SET receive_order=?, doc_date=?, doc_no=?, description=?, transaction_type=?, amount=?, file_name=? WHERE id=?");
            $stmt->bind_param("issssdsi", $receive_order, $doc_date, $doc_no, $description, $transaction_type, $amount, $file_name, $id);
        } else {
            $stmt = $conn->prepare("UPDATE receive_national SET receive_order=?, doc_date=?, doc_no=?, description=?, transaction_type=?, amount=? WHERE id=?");
...
// --- ดึงข้อมูล ---
$sql_data = "SELECT * FROM receive_national ORDER BY receive_order ASC";
...
<head>
    ...
    <title>ทะเบียนรับเงินรายได้แผ่นดิน - AMSS++</title>
    ...
    <style>
        ...
        .table-custom td { 
            ...
            padding: 8px; 
            font-size: 0.85rem; 
            background-color: white !important; /* พื้นหลังขาว */
        }
        
        /* แถวรวมยอด (Footer Row) */
        .total-row {
            background-color: #ffcccc !important; /* สีชมพูสำหรับแถวรวม */
            font-weight: bold;
            color: #333;
        }
...
<body>
    ...
    <div class="sub-header">ทะเบียนรับเงินรายได้แผ่นดิน</div>
    ...
    <div class="container-fluid pb-5 px-4">
        <div class="content-card">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <h2 class="page-title m-0">ทะเบียนรับเงินรายได้แผ่นดิน ปีงบประมาณ 2568</h2>
                ...
                <form action="Receivenational.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="doc_no" id="doc_no" class="form-control" placeholder="เช่น 7 ก 17121/3" required>
                        </div>
                        <div class="mb-3">
