...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $amount_request = $_POST['amount_request'];
    $amount_tax = $_POST['amount_tax'];
    // คำนวณรับจริงอัตโนมัติ (ขอเบิก - ภาษี)
    $amount_net = $amount_request - $amount_tax;

    if (isset($_POST['action']) && $_POST['action'] == 'add') {
        $stmt = $conn->prepare("INSERT INTO treasury_withdrawals (withdrawal_order, doc_date, petition_no, doc_no, period_no, description, amount_request, amount_tax, amount_net) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isssssddd", $withdrawal_order, $doc_date, $petition_no, $doc_no, $period_no, $description, $amount_request, $amount_tax, $amount_net);
        $stmt->execute();
    } elseif (isset($_POST['action']) && $_POST['action'] == 'edit') {
        $id = $_POST['edit_id'];
        $stmt = $conn->prepare("UPDATE treasury_withdrawals SET withdrawal_order=?, doc_date=?, petition_no=?, doc_no=?, period_no=?, description=?, amount_request=?, amount_tax=?, amount_net=? WHERE id=?");
        $stmt->bind_param("isssssdddi", $withdrawal_order, $doc_date, $petition_no, $doc_no, $period_no, $description, $amount_request, $amount_tax, $amount_net, $id);
        $stmt->execute();
    }
    header("Location: TreasuryWithdrawal.php");
    exit();
...
// --- ดึงข้อมูล ---
$sql_data = "SELECT * FROM treasury_withdrawals ORDER BY withdrawal_order ASC";
...
<head>
    ...
    <title>ทะเบียนขอเบิกเงินคงคลัง - AMSS++</title>
    ...
    <style>
        ...
        .btn-edit { color: #ffc107; font-size: 1.1rem; }
...
<body>
    ...
    <div class="sub-header">ทะเบียนขอเบิกเงินคงคลัง</div>
    ...
    <div class="container-fluid pb-5 px-3">
        <div class="content-card">
            ...
            <h2 class="page-title">ทะเบียนขอเบิกเงินคงคลัง ปีงบประมาณ 2568</h2>
                    ...
                    <tbody>
                        ...
                        if ($result_data->num_rows > 0) {
                            while($row = $result_data->fetch_assoc()) {
                                ...
                                echo "<td class='td-center'>" . $row['petition_no'] . "</td>";
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header d-block">
                    <h5 class="modal-title-custom" id="modalTitle">ลงทะเบียน ขอเบิกเงินคงคลัง ปีงบประมาณ 2568</h5>
                </div>
                <div class="modal-body form-yellow-bg mx-3 mb-3">
                    <form action="TreasuryWithdrawal.php" method="POST">
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-md-2 form-label-custom">เลขที่เอกสาร</div>
                            <div class="col-md-3">
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-md-3">
                                <input type="number" step="0.01" name="amount_request" id="amount_request" class="form-control form-control-sm" required>
                            </div>
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-md-3">
                                <input type="number" step="0.01" name="amount_tax" id="amount_tax" class="form-control form-control-sm" value="0.00" required>
                            </div>
                            <div class="col-md-1">บาท</div>
                        </div>
    ...
    <script>
        function openAddModal() {
            ...
            document.getElementById('modalTitle').innerHTML = 'ลงทะเบียน ขอเบิกเงินคงคลัง ปีงบประมาณ 2568';
