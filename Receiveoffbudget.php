...
if (isset($_GET['delete_id'])) {
    ...
    // ลบไฟล์จริง
    $sql_file = "SELECT file_name FROM receive_off_budget WHERE id = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'add') {
        ...
        if ($file_name) {
            $q = $conn->query("SELECT file_name FROM receive_off_budget WHERE id=$id");
            $old = $q->fetch_assoc();
            if(!empty($old['file_name']) && file_exists("uploads/".$old['file_name'])){
                unlink("uploads/".$old['file_name']);
            }
            
            $stmt = $conn->prepare("UPDATE receive_off_budget SET receive_order=?, doc_date=?, doc_no=?, description=?, transaction_type=?, amount=?, file_name=? WHERE id=?");
            $stmt->bind_param("issssdsi", $receive_order, $doc_date, $doc_no, $description, $transaction_type, $amount, $file_name, $id);
        } else {
            $stmt = $conn->prepare("UPDATE receive_off_budget SET receive_order=?, doc_date=?, doc_no=?, description=?, transaction_type=?, amount=? WHERE id=?");
...
<head>
    ...
    <style>
        ...
        .total-row {
            background-color: #fff3cd !important; /* สีเหลืองอ่อนจางๆ สำหรับแถวรวม */
            font-weight: bold;
            color: #333;
        }
...
<body>
    ...
    <div class="sub-header">ทะเบียนรับเงินนอกงบประมาณ</div>
    ...
    <div class="modal fade" id="addModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="Receiveoffbudget.php" method="POST" enctype="multipart/form-data">
