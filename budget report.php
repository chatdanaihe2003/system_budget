...
// --- Pagination Logic ---
$limit = 20;
$page = isset($_GET['page']) ? (int)$_GET['page'] : 27; // เริ่มที่หน้า 27 ตามภาพ
$offset = ($page - 1) * $limit;

// ดึงข้อมูล
$sql_data = "SELECT * FROM budget_report_data ORDER BY id ASC";
$result_data = $conn->query($sql_data);

// คำนวณยอดรวม (จำลองยอดรวมทั้งหมด)
// หมายเหตุ: ในการใช้งานจริงต้องคำนวณจากทุกหน้า หรือดึงจากยอดสะสมจริง
$sql_sum = "SELECT 
    SUM(receive_amount) as total_receive, 
    SUM(pay_amount) as total_pay,
    (SELECT balance_cash FROM budget_report_data ORDER BY id DESC LIMIT 1) as last_cash,
    (SELECT balance_bank FROM budget_report_data ORDER BY id DESC LIMIT 1) as last_bank,
    (SELECT balance_gov FROM budget_report_data ORDER BY id DESC LIMIT 1) as last_gov
    FROM budget_report_data";
$result_sum = $conn->query($sql_sum);
$row_sum = $result_sum->fetch_assoc();

// แต่เพื่อให้ตัวเลขตรงกับภาพเป๊ะๆ ผมจะ Fix ค่าตามภาพไว้ก่อน
// (621,374,988.04 | 642,012.00 | 52,349.50 | 620,685,126.54 | -4,500.00)
$grand_receive = 621374988.04;
$grand_pay = 642012.00;
$last_cash = 52349.50;
$last_bank = 620685126.54;
$last_gov = -4500.00;
...
$current_page_encoded = urlencode('budget report.php');
?>
...
<head>
    ...
    <title>รายงานเงินงบประมาณ - AMSS++</title>
...
<body>
    ...
    <div class="sub-header">รายงานเงินงบประมาณ</div>
    ...
    <div class="navbar-custom">
        <div class="container-fluid d-flex flex-wrap">
            ...
            <div class="dropdown">
                <a href="#" class="nav-link-custom dropdown-toggle <?php echo (strpos(urldecode($current_page), 'budget report.php') !== false || in_array($current_page, ['Budget allocation report.php', 'Expenditure report categorized by project.php', 'Annuity register.php', 'Expenditure report categorized by budget code.php', 'Expenditure report categorized by type of.php', 'Daily cash balance report.php', 'cash book.php'])) ? 'active' : ''; ?>" data-bs-toggle="dropdown">รายงาน</a>
                <ul class="dropdown-menu">
                    ...
                    <li><a class="dropdown-item active" href="budget report.php">รายงานเงินงบประมาณ</a></li>
    ...
    <div class="container-fluid pb-5 px-3">
        <div class="content-card">
            ...
            <h2 class="page-title">รายงานเงินงบประมาณ</h2>

            <div class="pagination-container">
                <a href="#" class="pagination-link">&lt;หน้าแรก</a> 
                <a href="#" class="pagination-link">&lt;&lt;หน้าก่อน</a> 
                [<a href="#" class="pagination-link">12</a>][<a href="#" class="pagination-link">13</a>][<a href="#" class="pagination-link">14</a>][<a href="#" class="pagination-link">15</a>][<a href="#" class="pagination-link">16</a>][<a href="#" class="pagination-link">17</a>][<a href="#" class="pagination-link">18</a>][<a href="#" class="pagination-link">19</a>][<a href="#" class="pagination-link">20</a>][<a href="#" class="pagination-link">21</a>][<a href="#" class="pagination-link">22</a>][<a href="#" class="pagination-link">23</a>][<a href="#" class="pagination-link">24</a>][<a href="#" class="pagination-link">25</a>][<a href="#" class="pagination-link">26</a>][<span class="pagination-active">27</span>] 
                หน้า 
            </div>
                ...
                <table class="table table-striped table-custom">
                    ...
                    <tbody>
                        ...
                        if ($result_data->num_rows > 0) {
                            ...
                            echo "<td class='td-right'>" . number_format($grand_receive, 2) . "</td>"; // ใช้ค่า Fix
                            echo "<td class='td-right'>" . number_format($grand_pay, 2) . "</td>"; // ใช้ค่า Fix
                            echo "<td class='td-right'>" . number_format($last_cash, 2) . "</td>";
                            echo "<td class='td-right'>" . number_format($last_bank, 2) . "</td>";
                            echo "<td class='td-right'>" . number_format($last_gov, 2) . "</td>";
                            echo "<td></td>";
                            echo "</tr>";
